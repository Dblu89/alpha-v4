...
def calcular_indicadores(df):
    ...
    for p in [5, 7, 10, 15, 20, 30, 50]:
        ...
        for s_int, s_float in [(10, 1.0), (12, 1.2), (15, 1.5), (18, 1.8), (20, 2.0), (25, 2.5), (30, 3.0)]:
            up  = sma + s_float * std
            lo  = sma - s_float * std
...
def gerar_sinais(familia, ind, params):
    ...
    if familia == "ema_crossover":
        ...
        if d == "long":
            ent = (rsi < lvl)        & (np.roll(rsi, 1) >= lvl)        & filt
            ext = rsi > 50
        else:
            ent = (rsi > (100 - lvl)) & (np.roll(rsi, 1) <= (100 - lvl)) & (~filt)
            ext = rsi < 50
        ent[0] = ext[0] = False
        ...
        rsi  = ind.get(f"rsi_{params['rsi_period']}")
        filt = params["rsi_filter"]
        if hist is None or rsi is None:
            return None, None
        if d == "long":
            ent = (hist > 0) & (np.roll(hist, 1) <= 0) & (rsi > filt)
            ext = (hist < 0) & (np.roll(hist, 1) >= 0)
        else:
            ent = (hist < 0) & (np.roll(hist, 1) >= 0) & (rsi < (100 - filt))
            ext = (hist > 0) & (np.roll(hist, 1) <= 0)
        ent[0] = ext[0] = False
        ...
        ovs   = params["oversold"]
        if d == "long":
            ent = (k < ovs)         & (np.roll(k, 1) >= ovs)         & filt
            ext = k > 50
        else:
            ent = (k > (100 - ovs)) & (np.roll(k, 1) <= (100 - ovs)) & (~filt)
            ext = k < 50
        ent[0] = ext[0] = False
        ...
        above = cls > ema
        if d == "long":
            ent = (rsi < lvl)        & above   & (hist > 0)
            ext = (rsi > 50)         | (hist < 0)
        else:
            ent = (rsi > (100 - lvl)) & (~above) & (hist < 0)
            ext = (rsi < 50)          | (hist > 0)
...
GRIDS = {
    # 1. RSI — maior familia com session (1.029.600 combos)
    "rsi_reversion": {
    ...
    # 3. EMA Crossover com session (15.876)
    "ema_crossover": {
    ...
    # 9. Volatility com session (1.350)
    "volatility": {
